...
** Syntax Highlighting & Visual Enhancements
   ...
   Honorable mention:
    ...
    - [[https://github.com/Fanael/highlight-defined][highlight-defined]] which is ok but superseded by =lisp-extra-font-lock= functionalities.
    - [[https://github.com/tarsius/paren-face][paren-face]] can be used to dim parentheses which some might find useful when dealing with Elisp.
    - [[https://github.com/purcell/page-break-lines][page-break-lines]] can be used for nicer display of form feed characters which are traditionally used to separate code sections in Elisp.
...
*** Relevant Content: Development Tools
    ...
    It's very tempting to list stuff such as =projectile= or =treemacs= but those package apply not only to Elisp development and should be listed in .

    Don't create a sub-section tools that have modern counterpart and are deprecated / no more maintained. You could list in


*** Relevant Content: Libraries

    In [[#libraries][Libraries]], only list package that were created to be used as libraries and generic enough to target broad range of applications.

    E.g. HTTP client libs such as =request= have their place, wrappers targeting a specific API (such as =ghub=) don't.
